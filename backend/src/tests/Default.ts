...
adminUser.salt =
	"823b5290ad76a63592760753b0c683bb2769f4d27f391b355ae45176d9a5725c";
adminUser.hash =
	"9b0682468581255f592253395146fae1cfcc22bb4a41ba71444329e6d572996bd4596d94f6ff4a51dbabf0d2c09ce496032e6b610df6a402d3bb70fcae49b2aa";
adminUser.educator = {
	admin: true,
	id: 1,
	subjects: [],
...
export const subjects = [1, 2, 3, 4].map((id) => {
	...
	subject.lessons = [1, 2, 3].map((id) => {
		...
		const computedId = id + (subject.id - 1) * 3;
...
export const virtualEntities = [1, 2, 3].map((id) => {
	...
	virtualEntity.quiz = {
		...
		questions: [1, 2, 3].map((id) => {
			...
			const computedId = id + (virtualEntity.id - 1) * 3;
			question.id = computedId;
			question.question = `Test Question ${computedId}`;
			question.correctAnswer = `Test Answer ${computedId}`;
			question.options = [
				`Test Option ${computedId}`,
				`Test Option ${computedId + 1}`,
				`Test Option ${computedId + 2}`,
				`Test Option ${computedId + 3}`,
			];
