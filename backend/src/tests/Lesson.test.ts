...
describe.skip('Lesson API tests', () => {
    ...
    describe('POST /lesson/createLesson', () => {
        it('should create a new lesson', async () => {
            ...
            when(App.mockedUserRepository.findOne(anything())).thenResolve(Default.educatorUser);
            when(App.mockedSubjectRepository.findOne(anyNumber(), anything())).thenResolve(Default.subjects[0]);
            when(App.mockedLessonRepository.save(anyOfClass(Lesson))).thenResolve({id: 1})
