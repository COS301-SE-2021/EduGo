...
router.post('/hidden/seed', async (req, res) => {
    ...
    let users: User[] = seed.user.map((value, index) => {
        ...
        user.organisation = organisations[Math.floor(Math.random() * orgLength)]
        return user;
    })
...
router.get('/hidden/seed', async (req, res) => {
    ...
    organisationRepository.findByIds([1, 2], {relations: ['users', 'users.student', 'users.educator']}).then(orgs => {
