...
export class MailgunEmailService implements EmailService {
    ...
    constructor() {
        ...
        let rawVerificationReminderTemplate = fs.readFileSync(`${__dirname}/templates/VerificationReminder.hbs`, 'utf-8');
        let rawAddedToSubjectTemplate = fs.readFileSync(`${__dirname}/templates/AddedToSubject.hbs`, 'utf-8');
    ...
    async SendBulkVerificationReminderEmails(emails: VerificationEmail[]): Promise<boolean> {
        return this.SendBulkTemplateEmail('reminder', emails);
    }

    async SendBulkAddedToSubjectEmails(emails: AddedToSubjectEmail[]): Promise<boolean> {
        return this.SendBulkTemplateEmail('added', emails);
    }

    private async SendBulkTemplateEmail(type: EmailType, content: VerificationEmail[] | AddedToSubjectEmail[]): Promise<boolean> {
        if (content.length === 0)
            return true;

        let recipientEmails: string = content.map(value => value.email).join(', ');
        ...
        if (type === 'verification' && this.isVerificationEmail(content)) {
            ...
            for (let i = 0; i < content.length; i++) {
                recipientJSON[content[i].email] = {html: this.verificationCodeTemplate({code: content[i].code, link: ''})}
            }
        }
        else if (type === 'reminder' && this.isVerificationEmail(content)) {
            subject = 'Just a little reminder';
            for (let i = 0; i < content.length; i++) {
                recipientJSON[content[i].email] = {html: this.verificationReminderTemplate({code: content[i].code, link: ''})}
            }
        }
        else if (type === 'added' && this.isAddedToSubjectEmail(content)) {
            subject = 'Another subject'
            for (let i = 0; i < content.length; i++) {
                recipientJSON[content[i].email] = {html: this.addedToSubjectTemplate({name: content[i].name, subject: content[i].subject})}
            }
        }
        else return false;
        ...
        return this.mg.messages().send(data).then(result => true).catch(err => false);
    ...
    private isAddedToSubjectEmail(object: any[]): object is AddedToSubjectEmail[] {
