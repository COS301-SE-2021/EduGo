...
export async function createSubject(request: CreateSubjectRequest){
    ...
    if(request.title== null||request.educatorId == null|| request.description == null){
        ...
        return subjectRepository.save(subject).then(value => {
            ...
            statusRes.message = 'There was an error adding the subject to the database'
            statusRes.type = 'fail'
            return statusRes;
        })
...
export async function GetSubjectsByEducator(request: GetSubjectsByEducatorRequest)  {
    ...
    if(request.educatorId==null)
    {
        ...
        let conn = await createConnection();
        let subjectRepository = conn.getRepository(Subject);
        subjectRepository.find({where:{educatorId:request.educatorId}} ).then(subjects => {

        if (subjects){
            let LessonsData: GetLessonsBySubjectResponse = {data:subjects,statusMessage:"Successful"} 
            return LessonsData;
        }
            
        }).catch(() => {
            statusRes.message = 'There was an error getting subjects for educators provided'
            statusRes.type = 'fail'
            return statusRes;
        })  
    }
