...
class AnswerParam extends RouterParam {
  ...
  var answers;
  var correctAnswers;
  AnswerParam(this.displayAnswers, this.answers, this.correctAnswers);
}
...
class _QuizPageState extends MomentumState<QuizPage> {
  ...
  late List<Widget> listOfWidgets;
  ...
  Widget build(BuildContext context) {
    MomentumBuilder _momentumBuilder = MomentumBuilder(
        ...
        builder: (context, snapshot) {
          ...
          //Dynamically create dropdown so options can be displayed dynamically
          var selectedAnswers = new Map();
          allOptionalAnswers.values.forEach((v) {
            v.insert(0, "Please select answer");
            //answerController.updateAnswer(v[0]);
            var answerModel = snapshot<AnswerPageModel>();
            listOfWidgets.add(new DropdownButton<String?>(
                value: answerModel.answer,
              ...
              child: ElevatedButton(
                onPressed: () {
                  MomentumRouter.goto(
                    ...
                    params:
                        AnswerParam(true, selectedAnswers, allCorrectAnswers),
                  );
                },
                child: const Text('Submit Answers'),
              ),
